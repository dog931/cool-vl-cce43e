...
function resolveBareFilePlacementHints(cwd: string, topFile: string | undefined, namedFiles: string[]): string[] {
	...
	if (newBareFiles.length === 0) return [];
	...
	return newBareFiles.slice(0, 8).map((file) => `${file} -> ${baseDir}/${file}`);
}
...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		const bareFilePlacementHints = resolveBareFilePlacementHints(cwd, topFile, namedFiles);
		if (fileHits.size === 0 && literalPaths.length === 0 && bareFilePlacementHints.length === 0) return "";
		...
		if (bareFilePlacementHints.length > 0) {
			sections.push("\nNEW FILE PLACEMENT hint (bare filenames only):");
			for (const hint of bareFilePlacementHints) {
				sections.push(`- ${hint}`);
			}
...
8. **git diff must be non-empty:** the harness runs \`git diff HEAD\` after your session. If it is empty your edits were no-ops, were reverted, or targeted the wrong path. The harness will detect this, reset your edit state, and force another pass — so verify your changes actually landed before stopping.
9. When done, stop. Your working-tree changes will be the final solution.

Your tools: read, edit, write, grep, find, ls, bash.

`;

const TAU_SCORING_PREAMBLE_OLD = `# Diff Overlap Optimizer
